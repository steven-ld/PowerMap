//! 暴露侧（expose，原 server 角色）运行逻辑。
//!
//! 部署在内网设备上：对外用 iroh 中继暴露一个 ALPN 服务；收到接入方（access）的
//! 连接后，按握手头里的"目标主机:端口"认证 token、按该客户白名单校验、在内网拨号
...
pub async fn run(
    ...
    if tokio::time::timeout(Duration::from_secs(online_timeout), endpoint.online())
        .await
        .is_err()
    {
        tracing::warn!(
